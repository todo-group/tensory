...
`TensorRepr`自体はあくまで表現であって、操作の実装は`XxxCtx`で定義する。
...
足に対して追加の情報を用いて管理を行う`AxisMapper`を持つ。

#### AxisMapper

足に対する管理を行う。`AxisMapper`は管理対象の`TensorRepr`の足の本数を変えない。
...
`Tensor`にさらに演算に応じてXxxCtxを供給するruntimeへの不変参照を張っている(可変性は内部可変を用いること)。
